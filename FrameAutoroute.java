...
public class FrameAutoroute extends JFrame {
    ...
    private class Ecouteur implements ActionListener
    {
        public void actionPerformed (ActionEvent ae)
        {
            ...
            //  Créer un objet Decimal Format qui permettra le formatage
                  // Formateur pour afficher le coût avec deux décimales
            DecimalFormat df = new DecimalFormat("###,##0.00");

            // Si c'est une route
            if (boutonRoute.isSelected()) {
                Route r = new Route(nbKilo, nbPo, nbPass, acco);
                cout = r.calculerCout();
            }
            // Si c'est une autoroute
            else {
                int nbEchan = Integer.parseInt(champNbEchangeurs.getText());
                Autoroute a = new Autoroute(nbKilo, nbPo, nbPass, nbEchan);
                cout = a.calculerCout();
            }

            // Afficher le coût dans le champ prévu
            champCout.setText(df.format(cout));
        } catch (NumberFormatException e) {
            champCout.setText("Erreur de saisie");
        }
    }
